...
export default function EditLinksView() {
  ...
  useEffect(() => {
    if (siteConfig && siteConfig.friends_content) {
      setContent(siteConfig.friends_content)
    }
  }, [siteConfig])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.put('/api/site-config', { friends_content: content })
      refetchConfig()
      navigate('/links')
    } catch (err: any) {
    ...
    <form onSubmit={handleSubmit} className="flex flex-col min-h-[calc(100dvh-140px)] md:h-[calc(100vh-140px)] h-auto max-w-7xl w-full mx-auto px-5 gap-6 pb-10 md:pb-0">
      <div className="flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold tracking-tight">编辑友情链接</h1>
        </div>
        <button
          type="submit"
          disabled={loading}
      ...
      <MarkdownEditor
        content={content}
        onChange={setContent}
        placeholder="输入友情链接的 Markdown 文本内容..."
      />
    </form>
  )
}
